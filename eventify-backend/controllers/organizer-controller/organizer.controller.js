...
exports.updateBookingStatusByOrganizer = async (req, res) => {
  try {
    if (!req.user || req.user.role !== "ORGANIZER") {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only ORGANIZER can update booking status.",
      });
    }
    ...
    try {
      // Find organizer's booked event
      const organizer = await Organizer.findById(req.user._id).session(session);
      if (!organizer) {
      ...
      const bookedEvent = organizer.bookedEvents.find(
      ...
      const previousBookingStatus = bookedEvent.bookingStatus;
      const previousPaymentStatus = bookedEvent.paymentStatus;

      // Update statuses
      if (bookingStatus) bookedEvent.bookingStatus = bookingStatus;
      if (paymentStatus) bookedEvent.paymentStatus = paymentStatus;

      await organizer.save({ session });

      // Optionally update TicketBooking collection as well
      const booking = await TicketBooking.findById(bookingId).session(session);
      if (booking) {
        if (bookingStatus) booking.bookingStatus = bookingStatus;
        if (paymentStatus) booking.paymentStatus = paymentStatus;
        await booking.save({ session });

        // Update ticket sold count if booking confirmed
        if (
    ...
    console.error("❌ Error updating booking status by organizer:", error);
    res.status(500).json({ success: false, message: "Server Error" });
  }
};
